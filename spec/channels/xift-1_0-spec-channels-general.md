...
|-----|------------------------------------------------------|----------------------------------------------------|--------------------------------------------------------------|------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|-----|----------------------------------------|
| C20 | `Channel5Handler`                                    | `SDRQueryReceived`                                 | n/a                                                          | Round-trip, signature verified, redaction honest           | `sdr_query_size_max`, `sdr_preview_size_max`                                                          | —   | SDR query/response                     |
| C21 | `Channel5Handler`, multiple `Receiver`s              | `SDRQueryReceived` to N peers                      | n/a                                                          | Top-K aggregation, deduplication                           | `sdr_max_results_hard_cap`                                                                            | —   | SDR scatter-gather                     |
| C22 | `PolicyZen`                                          | `SDRCandidateScored`                               | n/a                                                          | Four-dimensional composite score is consistent             | (no numeric)                                                                                          | —   | Composite scoring                      |
...
| C35 | `Channel4Handler`                                    | WebSocket upgrade on `/notifications/ws`           | `Opening` → `Streaming`                                      | Successful upgrade when `"ws"` advertised; events delivered as WS text frames; ack received | (no numeric)                                                              | —   | WebSocket escalation                   |
